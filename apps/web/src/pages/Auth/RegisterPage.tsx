...
import { registerSchema } from "@suzume/validation";
...
export default function RegisterPage() {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    setLoading(true);
    try {
      const needsPreparationSetup = await register(result.data);
      navigate(needsPreparationSetup ? "/preparation-setup" : "/dashboard");
    } catch (err) {
      setFormError(err instanceof ApiError ? err.message : "Unable to register. Please try again.");
    } finally {
      setLoading(false);
    }
  }
    ...
    <div className="flex min-h-screen flex-col items-center justify-center bg-[#f7f7fb] px-4">
      ...
      <div className="w-full max-w-sm rounded-2xl border border-slate-200 bg-white p-8 shadow-card">
        <div className="mb-6 flex flex-col items-center gap-2 text-center">
          ...
          <p className="text-sm text-slate-400">Start tracking your placement journey.</p>
        </div>
